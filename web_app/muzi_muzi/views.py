...
class LatestViewMixIn(object):
    ...
    def latest(self, request):
        ...
        if not isinstance(latest_num, int) and not latest_num.isdigit():
            return Response(
                {'error': f'Wrong {self.latest_param} parameter, was not a number. ({self.latest_param}={latest_num})'})
        queryset = self.get_queryset().order_by('-' + self.latest_by)[:int(latest_num)]
        serializer = self.serializer_class(queryset, many=True, context={'request': request})
        return Response(serializer.data)


class ListAllMixIn(object):
    """
    Custom mixin for /all endpoint that returns all data without pagination.
    """
...
class ProfessionList(viewsets.ModelViewSet, ListAllMixIn):
    queryset = Profession.objects.all()
    serializer_class = ProfessionSerializer


class GenreList(viewsets.ModelViewSet, ListAllMixIn):
